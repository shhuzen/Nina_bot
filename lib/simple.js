...
${extraLinks
  .map((link, index) =>
    link
      ? `item${index + 5}.URL;type=pref:${link}\nitem${
          index + 5
        }.X-ABLabel:Extra Link ${index + 1}`
      : ""
  )
  .join("\n")}
${
  isi5
    ? `${
        extraLinks.length > 0 ? `item${extraLinks.length + 5}` : "item5"
      }.X-ABLabel:${isi5}`
    : ""
}
END:VCARD`.trim();
      ...
      async value(jid, text = "", quoted, options) {
        if (Buffer.isBuffer(text)) {
          ...
          let canalId = [
            ...
            "120363377833048768@newsletter",
            "120363220939514640@newsletter",
            "120363220939514640@newsletter",
            "120363368073378190@newsletter",
            "120363374486687514@newsletter",
            "120363183614708156@newsletter",
          ];
          let canalNombre = [
            "☆Ботон☆",
            "☆Ботон☆",
        ...
        if (jid.endsWith("@g.us")) {
          ...
          if (!(v.name || v.subject)) {
            return conn
              ...
              .then(
                (metadata) =>
                  metadata || store?.fetchGroupMetadata(jid, conn.groupMetadata)
              )
    ...
    isBaileys: {
      get() {
        return (
          ...
          (this.key.fromMe &&
            this.id.startsWith("3EB0") &&
            (this.id.length === 20 || this.id.length === 22)) ||
          false
        );
