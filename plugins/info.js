...
let handler = async (
  ...
  frep = {
    contextInfo: {
      externalAdReply: {
        title: wm,
        body: lenguajeGB.smsCreApoyo(),
        sourceUrl: redesMenu.getRandom(),
        thumbnail: await (await fetch(gataMenu.getRandom())).buffer(),
      },
    },
  };
  fkontak = {
  ...
  const isCommand1 =
    /^(estado|status|estate|state|stado|stats|botstat(us)?)$/i.test(command);
  const isCommand2 =
    /^(cuenta(s)?oficiales?|gataig|(cuentas|account)s?g?b|(ig)?gata|(cuentasgata))$/i.test(
      command
    );
  const isCommand3 =
    /^(gata(bot)?groups?|grupos(ofc|degatabot|gb)|grupogb|groupsgb|gatagroups?|grupos|group(ofc|sofc))$/i.test(
      command
    );
  const isCommand4 = /^(c(ó|o)digo|sc|git|script)$/i.test(command);
  const isCommand5 =
    /^(instalar(gata)?bot|proceso(del)?bot|bot(install|proceso)|installbot)$/i.test(
      command
    );
  const isCommand6 =
    /^(owner|creator|propietario|dueño|dueña|propietaria|dueño|creadora|creador)$/i.test(
      command
    );
  const isCommand7 =
    /^(group(s|list|o(lista)?)|list(a)?(de)?grupo(s)?|grupolista)$/i.test(
      command
    );
  const isCommand8 = /^(info(gata|bot)|informaci(ón|on)(gata|bot))$/i.test(
    command
  );
  const isCommand9 = /^(contactos?|contacts?)$/i.test(command);
  const isCommand10 = /^(ping|speed|velocidad|rapidez|velocity)$/i.test(
    command
  );
  const isCommand11 =
    /^(dona(te|si)|donar|apoyar|paypal|donating|aportar)$/i.test(command);
  const isCommand12 =
    /^(report|request|reporte|bugs|bug|reportowner|reportes|reportar)$/i.test(
      command
    );
    ...
    case isCommand4:
      try {
        ...
        await conn.sendFile(
          ...
          gataImg.getRandom(),
          "gata.jpg",
          `*_${_package.homepage}_*\n\n` + lenguajeGB.smsPrivadoDonar(),
          fkontak
        );
      } catch (e) {
        reportError(e);
...
git clone https://github.com/GataNina-Li/БотонLite-MD 
cd БотонLite-MD
yarn install
npm install
npm start`;
...
node .`;
      try {
        ...
        let replit = `*◎ R E P L I T*\n\nhttps://replit.com/github/GataNina-Li/БотонLite-MD`;
        let heroku = `*◎ H E R O K U*\n\nhttps://heroku.com/deploy?template=https://github.com/GataNina-Li/БотонMDLite-Heroku`;
...
https://classic.yarnpkg.com/en/docs/install#windows-stable`;
    ...
    case isCommand6:
      try {
        ...
        let pp = await conn
          .profilePictureUrl(
            `${global.conn.user.jid.split`@`[0]}` + "@s.whatsapp.net",
            "image"
          )
          .catch((_) => gataImg.getRandom());
        let aa = `*_🍄 ${lenguajeGB.smsCreA()} ${taguser}, ${lenguajeGB.smsCreB()} ${
          _package.name
        } ${lenguajeGB.smsCreC()}_*\n\n`;
    ...
    case isCommand9:
      try {
        let contact,
          ...
          enlace,
          biog;
        let biografiaBot = await conn
          .fetchStatus(conn.user.jid.split("@")[0] + "@s.whatsapp.net")
          .catch((_) => "undefined");
        let bioBot =
          biografiaBot.status?.toString() ||
          `${desc2 == "" ? lenguajeGB.smsContacto1() : desc2}`;
        ...
        for (let i = 0; i < contacts.length; i++) {
          ...
          let biografia = await conn
            .fetchStatus(number + "@s.whatsapp.net")
            .catch((_) => "undefined");
          let bio =
            biografia.status?.toString() ||
            `${desc2 == "" ? lenguajeGB.smsContacto2() : desc2}`;
          nombre =
            official[0][0] == String(contact[0])
              ? official[0][1]
              : official[1][0] == String(contact[0])
              ? official[1][1]
              : official[2][0] == String(contact[0])
              ? official[2][1]
              : official[3][0] == String(contact[0])
              ? official[3][1]
              : lenguajeGB.smsContacto3();
          description =
            official[0][0] == String(contact[0])
              ? "Solo temas de Ботон"
              : official[1][0] == String(contact[0])
          ...
          lista.push([
            ...
            nombre,
            description,
            official[3][0] == String(contact[0]) ? null : correo,
            lugar,
            enlace,
            bio,
            official[1][0] == String(contact[0])
        ...
        lista.push([
          ...
          mail === "" ? "centerБотон@gmail.com" : mail,
          lenguajeGB.smsContacto7(),
          md,
          bioBot,
          yt,
...
*» ${text}*`;
          setTimeout(async function () {
            ...
            await conn.sendMessage(
              "120363349916000764@newsletter",
              {
                text: m.quoted ? teks2 + m.quoted.text : teks2,
                contextInfo: {
                  externalAdReply: {
