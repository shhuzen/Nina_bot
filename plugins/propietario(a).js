...
let handler = async (
  ...
  const isCommand4 = /^((set|cambiar|nueva|new)(bio|botbio|biobot))$/i.test(
    command
  );
  const isCommand5 =
    /^((set|cambiar|nuev(a|o)?|new)(name|botname|namebot|nombre|nombrebot|botnombre))$/i.test(
      command
    );
  const isCommand6 =
    /^((set|cambiar|nueva|new)(ppbot|botpp|fotobot|botfoto))$/i.test(command);
  ...
  const isCommand10 = /^(restablecerdatos|borrardatos|deletedatauser)$/i.test(
    command
  );
  ...
  switch (true) {
    case isCommand1:
      ...
      if (conn.user.jid != global.conn.user.jid) {
        if (!fs.existsSync(`./Ботон/${conn.user.jid.split`@`[0]}/creds.json`)) {
      ...
      try {
        ...
        const path =
          conn.user.jid != global.conn.user.jid
            ? `./Ботон/${conn.user.jid.split`@`[0]}/creds.json`
...
reportError(e)
    ...
    case isCommand2:
      try {
        ...
        if (!text && !m.quoted)
          return conn.reply(m.chat, bant, null, { mentions: [user] });
        try {
          if (text) {
          ...
          for (let i = 0; i < global.owner.length; i++) {
            ...
            if (user.replace(/@s\.whatsapp\.net$/, "") === ownerNumber) {
              ...
              await conn.reply(
                m.chat,
                lenguajeGB.smsPropban3(ownerNumber),
                null,
                { mentions: [aa] }
              );
              return;
          ...
          await conn.reply(user, lenguajeGB.smsPropban6(number, usr), null, {
        ...
        await conn.reply(
          m.chat,
          lenguajeGB.smsPropban7(usedPrefix, command, number),
          null,
          m
        );
        console.log(e);
    ...
    case isCommand7:
      try {
        if (global.conn.user.jid == conn.user.jid) {
          let stdout = execSync(
            "git pull" + (m.fromMe && text ? " " + text : "")
          );
          await conn.reply(m.chat, stdout.toString(), m);
        }
      } catch {
        var update = execSync(
          "git remote set-url origin" + md + ".git && git pull"
        );
    ...
    case isCommand9:
      ...
      if (
        !((cmd === "unblock" || cmd === "desbloquear") && (isOwner || isROwner))
      ) {
        for (let i = 0; i < global.owner.length; i++) {
          ownerNumber = global.owner[i][0];
          if (who.replace(/@s\.whatsapp\.net$/, "") === ownerNumber) {
            ...
            await conn.reply(
              m.chat,
              lenguajeGB.smsBlockUn2(comd, ownerNumber),
              null,
              { mentions: [aa] }
            );
            return;
      ...
      if (res[0])
        conn.reply(m.chat, lenguajeGB.smsBlockUn3(comd, useB), m, {
          mentions: res,
        });
      break;
    ...
    case isCommand10:
      ...
      if (!text && !m.quoted)
        return conn.reply(m.chat, lenguajeGB.smsRestarU1(), m);
      ...
      try {
        ...
        conn.reply(m.chat, lenguajeGB.smsRestarU3(number), null, {
          mentions: [user],
        });
      }
      break;
    ...
    case isCommand11:
      ...
      let link =
        (m.quoted ? (m.quoted.text ? m.quoted.text : text) : text) || text;
    ...
    case isCommand12:
      if (!text && !m.quoted)
        return m.reply(lenguajeGB.smsBCMensaje(usedPrefix, command));
      ...
      let content = conn.cMod(
        m.chat,
        cc,
        /bc|broadcast/i.test(teks) ? teks : lenguajeGB.smsJBDifu1() + teks
      );
      for (let id of users) {
        await delay(1500);
      ...
      await conn.reply(
        m.chat,
        lenguajeGB.smsJBDifu2(numUser, difuUser, tolUser).trim(),
        m
      );
      break;
    ...
    case isCommand13:
      ...
      let año = d.toLocaleDateString(lenguajeGB.lenguaje(), {
        year: "numeric",
      });
      ...
      for (let i = 0; i < groups.length; i++) {
        const id = groups[i];
        const infoGP = lenguajeGB.smsChatGP2(
          readMS,
          dia,
          mes,
          año,
    ...
    case isCommand14:
      ...
      let chats = Object.keys(global.db.data.users).filter((user) =>
